...
def create_model(input_shape, num_classes):
    model = tf.keras.Sequential([
        layers.Conv1D(256, kernel_size=9, activation='relu', input_shape=input_shape),
        layers.MaxPooling1D(pool_size=2),
        #layers.Dropout(0.5),
        layers.Conv1D(256, kernel_size=9, activation='relu'),
        layers.MaxPooling1D(pool_size=2),
        #layers.Dropout(0.5),
        layers.Flatten(),
...
def create_model_6(input_shape, num_classes):
    ...
    output_layer = layers.Dense(num_classes, activation='sigmoid')(layer6)
...
for train, test in kfold.split(X, Y):
    ...
    checkpoint = ModelCheckpoint(checkpoint_path, monitor='val_loss', mode='auto', save_best_only=True)

    model = create_model_6((emb_dim, 1), 1)
    ...
    model.save('saved_model/CNN/model_0622_'+ str(k_num))
